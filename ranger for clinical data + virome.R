...
for (d in c(30, 90)) {
  for (i in 0:4) {
    train = read_csv(paste0("train6_", d, "_imp", i, ".csv"))
    test = read.csv(paste0("test6_", d, "_imp", i, ".csv"))
...
for (d in c(30, 90)) {
  for (i in 0:4) {
    ...
    if (d == 30) {
      train.sel = train[c(names(importance), "ThirtyMo")]
      test.sel = test[c(names(importance), "ThirtyMo")]
    } else {
      train.sel = train[c(names(importance), "NinetyMo")]
      test.sel = test[c(names(importance), "NinetyMo")]
    }
...
for (d in c(30, 90)) {
  for (i in 0:4) {
    ...
    write_csv(train.sel, paste0("train6_", d, "_sel", i, ".csv"))
    write_csv(test.sel, paste0("test6_", d, "_", i, ".csv"))
  }
}
