...
for (d in c(30, 90)) {
  for (i in 0:4) {
    train.raw = read.csv(paste0("train5_", d, "_raw", i, ".csv"))
    test.raw = read.csv(paste0("test5_", d, "_raw", i, ".csv"))
    
    train = read.csv(paste0("train5_", d, "_imp", i, ".csv"))
    test = read.csv(paste0("test5_", d, "_imp", i, ".csv"))
...
for (d in c(30, 90)) {
  for (i in 0:4) {
    ...
    write_csv(train.raw, paste0("train5_", d, "_raw", i, ".csv"))
    write_csv(test.raw, paste0("test5_", d, "_raw", i, ".csv"))
    
    write_csv(train.sel, paste0("train5_", d, "_sel", i, ".csv"))
    write_csv(test.sel, paste0("test5_", d, "_", i, ".csv"))
  }
}
